module RDF
  ...
  #
  # Repository implementations may provide support for transactional
  # updates by providing an atomic implementation of
  # {Mutable#apply_changeset} and responding to
  # `#supports?(:atomic_write)` with `true`.
  #
  # We carefully distinguish between read-only and read/write
  # transactions, in order to enable repository implementations to
  # take out the appropriate locks for concurrency
  # control. Transactions are read-only by default; mutability must be
  # explicitly requested on construction in order to obtain a
  # read/write transaction.
  #
  # Individual repositories may make their own sets of guarantees
  # within the transaction's scope. In case repository implementations
  # should be unable to provide full ACID guarantees for transactions,
  # that must be clearly indicated in their documentation. If update
  # atomicity is not provided, `#supports?(:atomic_write)` must
  # respond `false`.
  #
  ...
  #   end
  #
  # The base class provides an atomic write implementation depending
  # on {RDF::Changeset} and using {Changeset#apply}. Custom
  # {Repository} classes can implement a minimial write-atomic
  # transactions by overriding {#apply_changeset}.
  #
  # Reads within a transaction run against the live repository by
  # default ({#isolation_level} is `:read_committed`). Repositories
  # may provide support for snapshots by implementing
  # {Repository#snapshot} and responding `true` to
  # `#supports?(:snapshots)`. In this case, the transaction will use
  # the {RDF::Dataset} returned by {#snapshot} for reads
  # (`:repeatable_read`).
  #
  # For datastores that support transactions natively, implementation
  # of a custom {Transaction} subclass is recommended. The
  # {Repository} is responsible for specifying snapshot support and
  # isolation level as appropriate. Note that repositories may provide
  # the snapshot isolation level without implementing {#snapshot}.
  #
  ...
  class Transaction
    ...
    ##
    # Indicates whether the transaction includes changes relative to
    # the target repository's state at transaction start time.
    #
    # The response is guaranteed to be `true` if executing the
    # transaction against the original repository state would cause a
    # change. It may also return `true` in cases where the repository
    # would not change (e.g.  because the transaction would insert
    ...
    #
    # @return [Boolean] true if the transaction has mutated
    #   (insert/delete) since transaction start time
    #
    ...
    def statement?(*args)
      ...
      when 1 then read_target.statement?(*args)
          ...
          "wrong number of arguments (given #{args.length}, expected 0 or 1)"
      end
    end
    ...
    def execute
      raise TransactionError, 'Cannot execute a rolled back transaction. ' \
        'Open a new one instead.' if
        instance_variable_defined?(:@rolledback) && @rolledback
    ...
    #   with a fresh one. Other implementations may need to explictly
    #   rollback at the supporting datastore.
    #
    ...
    def query_pattern(*args, **options, &block)
      read_target.send(:query_pattern, *args, **options, &block)
    end

    def query_execute(*args, **options, &block)
      read_target.send(:query_execute, *args, **options, &block)
    end
    ...
    ##
    # @private Adds the default graph_name to the statement, when one
    ...
    class SerializedTransaction < Transaction
      ...
      ##
      # Replaces repository data with the transaction's snapshot in a
      # safely serializable fashion.
      #
      # @note this transaction uses a pessimistic merge strategy which
      #   fails the transaction if any data has changed in the
      #   repository since transaction start time. However, the
      #   specific guarantee is softer: multiple concurrent
      #   conflicting transactions will not succeed. We may choose to
      #   implement a less pessimistic merge strategy as a
      #   non-breaking change.
      #
    ...
    #
    # This error indicates that the transaction semantics have been
